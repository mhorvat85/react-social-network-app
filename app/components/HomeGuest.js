...
function HomeGuest() {
  ...
  useEffect(() => {
    if (state.username.checkCount) {
      ...
      async function fetchResults() {
        try {
          const response = await Axios.post("/doesUsernameExist", { username: state.username.value }, { signal });
  ...
  useEffect(() => {
    if (state.email.checkCount) {
      ...
      async function fetchResults() {
        try {
          const response = await Axios.post("/doesEmailExist", { email: state.email.value }, { signal });
